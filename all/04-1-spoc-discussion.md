...
1. 寄存器、高速缓存、内存、外存的访问特征？  
    从寄存器到外存，访问速度越来越慢，容量越来越大。

2. 如何理解计算机中的存储层次结构所的理想状态是“均衡繁忙”状态？  
    存储面临的是访问速度和容量的tradeoff，要想达到大容量且速度相对较快，就必须利用所有存储层次结构，使其均衡繁忙。

3. 在你写程序时遇到过内存不够的情况吗？尝试过什么解决方法？  
    遇到过。将数据存入一个文件，在需要的时候再访问文件加载数据。

### 8.2 覆盖和交换

1. 什么是覆盖技术？使用覆盖技术的程序开发者的主要工作是什么？   
    覆盖技术指应用程序手动把需要的指令和数据保存在内存中。程序开发者需要手动完成数据在内存和外存间的交换。
3. 什么是交换技术？覆盖与交换有什么不同？  
    交换技术指操作系统自动把暂时不能执行的程序保存到外存中。  
    不同有：  
      覆盖技术发生在没有调用关系的程序模块间，而交换技术发生在进程间，以进程为单位。覆盖技术需要程序员给出模块间的逻辑覆盖结构，而交换技术不需要。  


4. 覆盖和交换技术在现代计算机系统中还有需要吗？可能用在什么地方？  
...
1. 什么是时间局部性、空间局部性和分支局部性？  
    时间局部性：一条指令的一次执行和下次执行，一个数据的一次访问和下次
访问都集中在一个较短时期内。  
    空间局部性：当前指令和邻近的几条指令，当前访问的数据和邻近的几个数
据都集中在一个较小区域内。  
    分支局部性：一条跳转指令的两次执行，很可能跳到相同的内存位置。  
2. 如何提高程序执行时的局部性特征？  
   访问数组时（尤其是二维数组），需要注意访问顺序。
...
1. 什么是虚拟存储？它与覆盖和交换的区别是什么？它有什么好处和挑战？  
    虚拟存储指只把部分程序放到内存中，从而运行比物理内存大
的程序，与覆盖不同之处在于它的交换是由操作系统完成，与交换不同之处在于在内存和外存之间只交换进程的部分内容。好处在于可以获得比物理内存空间更大的逻辑地址空间，挑战在于需要一些较复杂的支持技术。
2. 虚拟存储需要什么样的支持技术？  
    硬件技术：需要硬件提供虚拟地址到物理地址的转换。  
    操作系统：操作系统需要管理内存和外存间的页或段的换入换出。

### 8.5 虚拟页式存储

 1. 什么是虚拟页式存储？缺页中断处理的功能是什么？  
    以固定大小的页为单位在内存与外存间换入换出，实现虚拟存储的功能。  
    将外存中的页换入内存，并选择一个页面被置换，再返回重新执行触发缺页中断的指令。

 2. 为了支持虚拟页式存储的实现，页表项有什么修改？  
    除了物理页帧号以外，还需要访问位、修改位、保护位和驻留位。

 3. 页式存储和虚拟页式存储的区别是什么？
    页式存储指以页为单位进行存储管理。虚拟页式存储是指用页式存储来实现虚拟存储功能。   
### 8.6 缺页异常
